...
/*@
  ...
  @ assigns \nothing;
  @*/
unsigned int I16_alt(UINT8 * ptr) {
...
/*@
  ...
  @ assigns \nothing;
  @*/
unsigned int I32_alt(UINT8 *ptr) {
...
ImagingFliDecode(Imaging im, ImagingCodecState state, UINT8* buf, Py_ssize_t bytes)
{
    ...
    //@ ghost Py_ssize_t bytes_original = bytes;
    ...
    framesize = I32_alt(ptr);
    if (framesize < I32_alt(ptr))
	return 0;

    /* Make sure this is a frame chunk.  The Python driver takes
       case of other chunk types. */

    if (I16_alt(ptr + 4) != 0xF1FA) {
	state->errcode = IMAGING_CODEC_UNKNOWN;
	return -1;
    }

    chunks = I16_alt(ptr + 6);
    ptr += 16;
    bytes -= 16;
    ...
    /*@
      ...
      @ loop invariant ptr > buf;
      @ loop invariant bytes <= bytes_original;
      ...
      @ loop assigns ptr, bytes, im->image[..][..], state->errcode, c, advance, i, x, y;
	...
	data = ptr + 6;
	switch (I16_alt(ptr + 4)) {
	case 4: case 11:
	    /* FLI COLOR chunk */
	    break; /* ignored; handled by Python code */
	case 7: BAIL_ON(1); // TODO
	    /* FLI SS2 chunk (word delta) */
	    lines = I16_alt(data); data += 2;
	    for (l = y = 0; l < lines && y < state->ysize; l++, y++) {
		UINT8* buf_alt = (UINT8 *) im->image[y];
		int p, packets;
		packets = I16_alt(data); data += 2;
	    ...
	    break;
	case 12: BAIL_ON(1); // TODO
	    /* FLI LC chunk (byte delta) */
	    y = I16_alt(data); ymax = y + I16_alt(data+2); data += 4;
	...
	case 13:
	    ...
	    /*@
	        loop invariant 0 <= y <= state->ysize;
	        loop assigns y, im->image[..][..];
	        loop variant state->ysize - y;
	      @*/
	    for (y = 0; y < state->ysize; y++)
		memset(im->image[y], 0, state->xsize);
	    break;
	case 15: BAIL_ON(1); // TODO
	    /* FLI BRUN chunk */
	    for (y = 0; y < state->ysize; y++) {
		UINT8* out = (UINT8*) im->image[y];
